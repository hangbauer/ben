...
ALTER TABLE `dodtl` 
ADD COLUMN `itemordersender` decimal(20,2) DEFAULT '0.00' AFTER `itemorder`;

ALTER TABLE `dodtl` 
ADD COLUMN `itemorderrecipient` decimal(20,2) DEFAULT '0.00' AFTER `itemordersender`;
...
BEGIN
	SELECT
        ...
        sender.name AS sendername, sender.address AS senderaddress, 
        bank.name AS bankname, bank.accountname, bank.accountno,
        case when mas.invoicetypeid = 0 and dodtl.itemordersender > 0 then dodtl.itemordersender when mas.invoicetypeid = 0 and dodtl.itemorderrecipient > 0 then dodtl.itemorderrecipient else dodtl.itemorder end itemorder2
	FROM invoicemas mas 
    INNER JOIN invoicedtl dtl ON dtl.invoicemasid = mas.id
    INNER JOIN domas ON domas.id = dtl.domasid
	INNER JOIN containertype con ON con.id = domas.containertypeid
